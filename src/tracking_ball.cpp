...
#define SPD (1.0)
#define SIKII (3000)    //抽出ピクセル数のしきい値
#define FRAME_WIDTH (640)   //フレームの横のピクセル数
#define FRAME_HEIGHT (480)  //フレームの縦のピクセル数
...
ros::Subscriber get_position = nh.subscribe<std_msgs::Float64MultiArray>("position_of_ball", 10, repos);    //ボールの位置（重心）
ros::Subscriber get_numpx = nh.subscribe<std_msgs::Int32>("pixel", 10, numpx);  //ピクセル数
ros::Publisher tracking = nh.advertise<geometry_msgs::Twist>("sub", 1); //動作指示
...
void repos(std_msgs::Float64MultiArray position){   //重心の更新
    pos = position;
}

void numpx(std_msgs::Int32 np){ //抽出ピクセル数の更新に応じて動作ステータスを変更
